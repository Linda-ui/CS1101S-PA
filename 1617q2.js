...
function all_different(nums) {
    ...
    const a = [];
    for (let r = 0;r < length(nums);r = r + 1) {
        const num = list_ref(nums, r);
        if (!is_undefined(a[num])) {
            return false;
        } else {
            a[num] = num;
        }
...
function is_valid_toto_set(nums, n, min, max) {
    function allInRange(nums, n, min, max) {
        for (let r = 0;r < n;r = r + 1) {
            const item = list_ref(nums, r);
    ...
    if (!(length(nums) === n)) {
        return false;
    } else if (!allInRange(nums, n, min, max)) {
        return false;
...
function num_of_matches(numsA, numsB) {
    const a = [];
    for (let r = 0; r < length(numsA); r = r + 1) {
        const item = list_ref(numsA, r);
        a[item] = item;
    }
    
    let count = 0;
    for (let r = 0; r < length(numsB); r = r + 1) {
        const n = list_ref(numsB,r);
...
function check_winning_group(bet_nums, draw_nums, extra_num) {
    ...
    if (length(bet_nums)!==n) {
        return 'Invalid ticket!';
    }
    
    const no = num_of_matches(bet_nums, draw_nums);
    const ex = num_of_matches(bet_nums, list(extra_num)) === 1;
    return no === n 
           ? 1
           : no === (n-1) && ex
           ? 2
